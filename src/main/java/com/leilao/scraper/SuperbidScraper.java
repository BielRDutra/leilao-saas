...
public class SuperbidScraper extends ScraperBase {
    ...
    private static LocalDateTime parsearData(String texto) {
        ...
        for (DateTimeFormatter fmt : List.of(FMT_COMPLETO, FMT_DATA)) {
            try {
                String t = texto.trim();
                if (fmt.equals(FMT_DATA)) {
                    return LocalDate.parse(t.substring(0, Math.min(t.length(), 10)), fmt).atStartOfDay();
                } else {
                    return LocalDateTime.parse(t.substring(0, Math.min(t.length(), 16)), fmt);
                }
